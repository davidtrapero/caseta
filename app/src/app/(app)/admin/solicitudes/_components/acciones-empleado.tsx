...
import { aprobarTurnosEmpleadoAction, rechazarTurnosEmpleadoAction } from "../actions";
...
export function AccionesEmpleado({ solicitudId, turnos }: Props) {
  ...
  const [aprobarState, aprobarAction, aprobarPending] = useActionState<
    ...
    FormData
  >(aprobarTurnosEmpleadoAction, null);

  const [rechazarState, rechazarAction, rechazarPending] = useActionState<
    ActionResult<{ solicitudId: string; rechazados: number }> | null,
    FormData
  >(rechazarTurnosEmpleadoAction, null);
      ...
      >
        <form action={rechazarAction} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            <label htmlFor={`motivo-emp-${solicitudId}`} className="text-sm font-medium">
              Motivo del rechazo
            </label>
            <textarea
              id={`motivo-emp-${solicitudId}`}
              name="motivo"
              rows={4}
              maxLength={500}
              value={motivo}
